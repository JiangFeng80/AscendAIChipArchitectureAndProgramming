...
public:
    ...
    */
    HIAI_StatusT RecvData(const std::shared_ptr<void>& message)
    {
        ...
        uint32_t compareResultListSize = 0;
        string tfile = "";
        for (auto compareResult : customOutput->compareResultList) {
            if (compareResultListSize < config::outputFileList.size()) {
                tfile += "Output file " + config::outputFileList[compareResultListSize] + " compare result ";
                tfile += (compareResult ? "true" : "false");
                tfile += "\n";
                compareResultListSize++;
            } else {
                HIAI_ENGINE_LOG(HIAI_IDE_ERROR, "compareResultListSize size is %d, outputFileList size is %d!", 
                                compareResultListSize, config::outputFileList.size());
                std::unique_lock <std::mutex> lck(localTestMutex);
                isTestResultReady = true;
                return HIAI_ERROR;
            }
        ...
        if (0 == customOutput->compareResultList.size()) {
            tfile += "None vertification result!\n";
            tfile += "If you prefer to vertify output(s), please set vertify configuration.\n";
        }
        if (WriteFile(vertifyResultFileName.c_str(), tfile.c_str(), tfile.size()) == -1 ) {
...
int OutputFileInit(std::string outputString, FILE *stream)
{
    ...
    for (unsigned int i = 0; i < outputString.length(); i++) {
        if (outputString[i] == ',') {
            if (ReadOutputFile(path, stream) != SUCCESS) {
                fprintf(stream, "[Error] Open file failed.\n");
                return FAILED;
            }
            path = "";
