...
@util.check_input_type((list, tuple), (list, tuple), str, str, str, int, int, int, int,
                       str, int, int)
def custom_convolution(shape_in, shape_w, in_dtype, w_dtype, res_dtype, padh, padw, strideh, stridew,
                   kernel_name="custom_convolution", need_build=0, need_print=0):
    """
...
#            else:
    ...
    config = {
        "print_ir": bool(need_print),
        "need_build": bool(need_build),
        "name": kernel_name,
